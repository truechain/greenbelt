...
async function start () {
  ...
  if (versionAlreadyExists) {
    ...
    await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' new ${VERSION}`)
    console.log(`removing any existing files from Sentry release "${VERSION}"...`)
    await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' files ${VERSION} delete --all`)
  }
  ...
  if (!versionHasArtifacts) {
    ...
    await exec(`for FILEPATH in ./dist/chrome/*.js; do [ -e $FILEPATH ] || continue; export FILE=\`basename $FILEPATH\` && echo uploading $FILE && sentry-cli releases --org 'greenbelt' --project 'greenbelt' files ${VERSION} upload $FILEPATH greenbelt/$FILE; done;`)
    console.log(`uploading sourcemaps Sentry release "${VERSION}"...`)
    await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' files ${VERSION} upload-sourcemaps ./dist/sourcemaps/ --url-prefix 'sourcemaps'`)
...
async function checkIfAuthWorks () {
  const itWorked = await doesNotFail(async () => {
    await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' list`)
  })
  return itWorked
}
...
async function checkIfVersionExists () {
  const versionAlreadyExists = await doesNotFail(async () => {
    await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' info ${VERSION}`)
  })
  return versionAlreadyExists
}

async function checkIfVersionHasArtifacts () {
  const artifacts = await exec(`sentry-cli releases --org 'greenbelt' --project 'greenbelt' files ${VERSION} list`)
