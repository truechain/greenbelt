...
describe('send selectors', () => {
  ...
  describe('getAmountConversionRate()', () => {
    ...
    it('should return the eth conversion rate if no token is selected', () => {
      const editedMockState = {
        greenbelt: Object.assign({}, mockState.greenbelt, { selectedTokenAddress: null }),
      }
      assert.equal(
  ...
  describe('getSelectedToken()', () => {
    ...
    it('should return the send token if none is currently selected, but a send token exists', () => {
      ...
      const editedMockState = {
        greenbelt: Object.assign({}, mockState.greenbelt, {
  ...
  describe('getSelectedTokenContract()', () => {
    ...
    it('should return null if no token is selected', () => {
      const modifiedGreenbeltState = Object.assign({}, mockState.greenbelt, { selectedTokenAddress: false })
      assert.equal(
        getSelectedTokenContract(Object.assign({}, mockState, { greenbelt: modifiedGreenbeltState })),
        null
      )
    })
  })
  ...
  describe('transactionsSelector()', () => {
    ...
    it('should return all transactions if no token is selected', () => {
      const modifiedGreenbeltState = Object.assign({}, mockState.greenbelt, { selectedTokenAddress: false })
      const modifiedState = Object.assign({}, mockState, { greenbelt: modifiedGreenbeltState })
      assert.deepEqual(
    ...
    it('should return shapeshift transactions if current network is 1', () => {
      const modifiedGreenbeltState = Object.assign({}, mockState.greenbelt, { selectedTokenAddress: false, network: '1' })
      const modifiedState = Object.assign({}, mockState, { greenbelt: modifiedGreenbeltState })
      assert.deepEqual(
        transactionsSelector(modifiedState),
        [
