 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows GreenBelt to use custom provider strategies for known networks.
 ...
 * @property {Array} trustedOriginList - trustedOriginList.
...
/**
 * Initializes the GreenBelt controller, and sets up all platform configuration.
...
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    if (diskStoreState && diskStoreState.data) {
      ...
      sentry.captureMessage('GreenBelt - Empty vault found - recovered from diskStore', {
      ...
      sentry.captureMessage('GreenBelt - Empty vault found - unable to recover')
    }
  }
  ...
  if (!versionedData) {
    throw new Error('GreenBelt - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('GreenBelt - Localstore not supported')
    })
  }
...
/**
 * Initializes the GreenBelt Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('GreenBelt - Persistence pipeline failed', error)
    }
  )
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('GreenBelt - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('GreenBelt - updated state does not have data', state)
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the GreenBelt controller via a multiplexed duplex stream.
   * This method identifies trusted (GreenBelt) interfaces, and connects them differently from untrusted (web pages).
  ...
  function connectRemote (remotePort) {
    ...
    if (isGreenBeltInternalProcess) {
      ...
      controller.setupTrustedCommunication(portStream, 'GreenBelt')
